...
const PYTHON_URL = process.env["PYTHON_SERVICE_URL"] as string;
...
app.use((req, _res, next) => {
  ...
  _res.on("finish", () => {
    console.log(
      `${C.gray}[http] ${req.method} ${req.path} ${_res.statusCode} (${Date.now() - start}ms)${C.reset}`,
    );
  });
  next();
});
...
app.post("/api/jobs", upload.any(), (req, res) => {
  ...
  if (midiFile) {
    ...
    fs.copyFileSync(
      midiFile.path,
      path.join(testInputDir, midiFile.originalname),
    );
    console.log(
      `${C.gray}[debug] saved midi file to ${path.join(testInputDir, midiFile.originalname)}${C.reset}`,
    );
  }
  if (audioFile) {
    const fs = require("fs");
    ...
    fs.copyFileSync(
      audioFile.path,
      path.join(testInputDir, audioFile.originalname),
    );
    console.log(
      `${C.gray}[debug] saved audio file to ${path.join(testInputDir, audioFile.originalname)}${C.reset}`,
    );
  }

  if (!midiFile && audioFile) {
    res.status(400).json({
      error: "Audio upload is no longer supported. Use MIDI recording.",
    });
    return;
  }
  if (!midiFile) {
  ...
  if (rawChords) {
    try {
      ...
      res
        .status(400)
        .json({ error: "chordsJson must be a JSON-encoded ChordEvent array" });
      return;
    }
  }
  ...
  console.log(
    `${C.bold}${C.cyan}[job:${jobId}]${C.reset} created file=${midiFile.originalname}`,
  );
  ...
  runPipeline(
    { midiPath, chords },
    { jobOutputDir, pythonServiceUrl: PYTHON_URL },
    (event) => {
      store.addEvent(jobId, event);
      if (event.type === "stage_start" && event.stage) {
        stageStart[event.stage] = Date.now();
        console.log(
          `${C.bold}${C.cyan}[job:${jobId}]${C.reset} ${C.yellow}▶ ${event.stage}${C.reset}`,
        );
      } else if (event.type === "stage_complete" && event.stage) {
        const elapsed = (
          (Date.now() - (stageStart[event.stage] ?? Date.now())) /
          1000
        ).toFixed(2);
        console.log(
          `${C.bold}${C.cyan}[job:${jobId}]${C.reset} ${C.green}✓ ${event.stage}${C.reset}${C.dim} (${elapsed}s)${C.reset}`,
        );
      } else if (event.type === "stage_error") {
        console.error(
          `${C.bold}${C.cyan}[job:${jobId}]${C.reset} ${C.red}✗ stage_error: ${event.error}${C.reset}`,
        );
      }
    },
  )
    .then((result) => {
      ...
      console.log(
        `${C.bold}${C.cyan}[job:${jobId}]${C.reset} ${C.green}${C.bold}complete${C.reset}`,
      );
    })
    .catch((err: unknown) => {
      ...
      console.error(
        `${C.bold}${C.cyan}[job:${jobId}]${C.reset} ${C.red}${C.bold}failed:${C.reset}${C.red} ${message}${C.reset}`,
      );
    });
