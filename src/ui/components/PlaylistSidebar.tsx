...
export function PlaylistSidebar({ onSongSelected }: Props) {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        const song =
          stored.songs.find((s) => s.title === stored.selectedTitle) ?? null;
        onSongSelected(song);
      }
    } catch {}
  ...
  const persist = (newSongs: Song[], newTitle: string | null) => {
    localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ songs: newSongs, selectedTitle: newTitle }),
    );
  };
  ...
  const handleImport = () => {
    ...
    try {
      ...
      const merged = [
        ...songs,
        ...parsed.filter((p) => !songs.some((s) => s.title === p.title)),
      ];
      setSongs(merged);
  ...
  return (
    <div className="w-44 h-screen border-r border-zinc-800 flex flex-col p-3 shrink-0">
      ...
      {songs.length > 0 && (
        <input
          ...
          type="text"
          placeholder="Search..."
          className="mb-3 p-1.5 text-xs bg-zinc-900 border border-zinc-700 rounded text-zinc-200"
        />
      )}
