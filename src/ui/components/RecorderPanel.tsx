...
export function RecorderPanel({ selectedSong, onJobCreated }: Props) {
  ...
  const handleRecord = async () => {
    ...
    try {
      ...
      notePollerRef.current = setInterval(() => setActiveNotes(recorder.getActiveNotes()), 50);
    } catch (e) {
  ...
  return (
    <div className="flex flex-col items-center justify-center flex-1 gap-4">
      ...
      <p
        ...
        style={{ animation: isRecording && activeNotes.length > 0 ? 'pulse 0.4s ease-in-out infinite alternate' : 'none' }}
      >
        {activeNotes.join(' · ')}
      </p>
